...
- [PHP](#php)
  ...
  - [8. Løkker](#8-løkker)
    ...
    - [8.2 for](#82-for)
    - [8.3 while](#83-while)
  - [9. Lese data fra URL](#9-lese-data-fra-url)
  - [10. Lese data fra HTML-skjemaer](#10-lese-data-fra-html-skjemaer)
  - [11. Vanlige brukerfeil i PHP](#11-vanlige-brukerfeil-i-php)
  - [12. Dynamiske nettsider med SQL og PHP](#12-dynamiske-nettsider-med-sql-og-php)
    - [12.1 Koble til en SQL-database med PHP](#121-koble-til-en-sql-database-med-php)
      - [12.1.1 MySQL](#1211-mysql)
      - [12.1.2 SQLite](#1212-sqlite)
...
> ### Oppgaver
>   1-1. Sett dere sammen to og to. En av dere skal nå forklare den første figuren ovenfor med egne ord, og så skal den andre forklare den neste figuren.
>   1-2. Se på appene du har på mobilen din. Tror du at noen av dem bruker klient-tjener-modellen? Gjør gjerne noen [nettsøk](https://www.google.no/search?q=client+server+examples+mobile+apps) for å forsøke å finne ut av det.
...
> ### Oppgaver
>  2-1. Lag en "Hallo verden"-side hvor du bare bruker HTML (ikke noe PHP-kode). Kall siden `hallo.html`, og legg den på en web-server.
>  2-2. Bruk nettet og finn ut når PHP ble til. Finn også ut hvilken versjon som er den nyeste.
>  2-3. Bruk nettet for å finne fem kjente nettsteder som bruker PHP.
...
> ### Oppgaver
>   3-1. Lag en "Hallo verden"-side hvor du bare bruker PHP. Kall siden `hallo.php`, og legg den på en web-server. Lag den slik at siden ser identisk ut med `hallo.html` som du laget tidligere.
>   3-2. Endre koden på siden din sånn at den skriver ut navnet ditt istedenfor "Hallo verden".
...
> ### Oppgaver
>   4-1. Ta utgangspunkt i siden `hallo.php` som du laget tidligere. Lag en variabel `$fornavn` som har fornavnet ditt som verdi. Endre koden din slik at siden nå viser resultatet av `echo "Hallo $fornavn"`. 
>   4-2. Ta utgangspunkt i forrige oppgave. Lag en variabel `$hilsen` og gi den verdien "Hei". Endre koden din slik at siden nå viser resultatet av `echo "$hilsen $fornavn"`. 
...
> ### Oppgaver
>   4-3. Bruk HTML-malen som ble vist i forrige kapittel. Kopier og lim den inn i en ny fil som du kaller `testfil.php`. Så tar du koden som demonstrerer bruk av matematiske operatorer ovenfor, og limer inn på riktig plass i malen. Åpne siden i en nettleser, og sjekk at alt fungerer. Forsøk så å endre på verdiene til de ulike variablene. Observer hva som skjer.
>   4-4. Ta utgangspunkt i eksempelkoden som summerer epler og pærer. Legg til en variabel for bananer, og gi den verdien 7. Legg til nødvendig kode for å regne ut det nye tallet for totalt antall frukt. Legg også til nødvendig HTML-kode slik at du får en pen nettside som resultat.
...
## 10. Lese data fra HTML-skjemaer
...
## 12. Dynamiske nettsider med SQL og PHP
...
### 12.1 Koble til en SQL-database med PHP
