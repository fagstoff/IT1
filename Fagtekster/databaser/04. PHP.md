...
> ### Oppgaver
>   1. Bruk HTML-malen som ble vist i forrige kapittel. Kopier og lim den inn i en ny fil som du kaller `testfil.php`. Så tar du koden som demonstrerer bruk av variabler ovenfor, og limer inn mellom `<pre><code>...</code></pre>`. Åpne siden i en nettleser, og sjekk at alt fungerer. Forsøk så å endre på verdiene til de ulike variablene. Observer hva som skjer.
>   2. Ta utgangspunkt i siden `hallo.php` som du laget tidligere. Lag en variabel `$fornavn` som har fornavnet ditt som verdi. Endre koden din slik at siden nå viser resultatet av `echo "Hallo $fornavn"`. 
